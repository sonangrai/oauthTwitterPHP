...
if(isset($_POST['savebtn']))
{
  ...
  //$temp = explode('.', $oldName);
    ...
    if(!is_dir($folder))
      mkdir($folder,777); 
      if(move_uploaded_file($_FILES['picture']['tmp_name'],$folder.$oldName))
        {
          if(updatephoto($conn,$_POST,$oldName,$twitterId))
            {
              redirection('account_edit.php');
            } 
        }
			...
			<div class="col-lg-4 col-md-6 col-12">
				<div class="imgprt">
					...
					<form class="flexer" method="POST" id="adminform" enctype="multipart/form-data">
						...
						<button type="submit" class="sbtn" name="savebtn">アップロード</button>
					</form>
				</div>
			</div>
